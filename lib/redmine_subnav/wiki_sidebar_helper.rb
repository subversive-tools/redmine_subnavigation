module RedmineSubnav
  module WikiSidebarHelper
    ...
    def render_headers(page)
      ...
      # Track seen anchors to handle duplicates (header, header-1, header-2)
      ...
      headers.each do |header|
        ...
        html << "<li><a href=\"#{Rails.application.routes.url_helpers.project_wiki_page_path(page.project, page.title, anchor: anchor)}\" class=\"wiki-header-link\" data-anchor=\"#{anchor}\">#{header}</a></li>"
      end
      html << '</ul>'
      html
    end
